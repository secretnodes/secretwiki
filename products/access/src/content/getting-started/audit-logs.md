...
Enterprise customers have access to detailed logs of HTTP requests, on their Cloudflare dashboard. Enterprise customers also have access to Cloudflare's Logpush service, which can be configured from the Cloudflare Dashboard or API (for more information about Cloudflare HTTP logging, see the [Cloudflare Logs](https://secret.wiki/logs/about/) section).
...
For instructions on setting up Logpush, see [Manage the Logpush API](https://secret.wiki/logs/logpush/logpush-configuration-api/).

For more on exporting per-request Access logs, see [Understanding the Logpush API](https://secret.wiki/logs/logpush/logpush-configuration-api/understanding-logpush-api/).
