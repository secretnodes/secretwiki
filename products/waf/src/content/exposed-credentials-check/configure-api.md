...
Configure exposed credentials checks using the [Rulesets API](https://secret.wiki/firewall/cf-rulesets/rulesets-api). You can deploy the Exposed Credentials Check Managed Ruleset and create custom rules that check for exposed credentials.
...
1. Use the [List existing rulesets](https://secret.wiki/firewall/cf-rulesets/rulesets-api/view#list-existing-rulesets) method to obtain the following ruleset IDs:
...
1. Use the [View ruleset](https://secret.wiki/firewall/cf-rulesets/rulesets-api/view#view-a-specific-ruleset) method to get the rules already associated with the Phase ruleset where you want to deploy the Managed Ruleset.
1. Use the [Update ruleset](https://secret.wiki/firewall/cf-rulesets/rulesets-api/update) method to add a rule to the Phase ruleset deploying the Exposed Credentials Check Managed Ruleset. Make sure you include the existing rules in the Phase ruleset in your `PUT` request.

For more information on deploying a Managed Ruleset, check [Deploy a Managed Ruleset](https://secret.wiki/firewall/cf-rulesets/managed-rulesets/deploy-managed-ruleset).
...
For more information on defining overrides for Managed Rulesets using the Rulesets API, check [Override a Managed Ruleset](https://secret.wiki/firewall/cf-rulesets/managed-rulesets/override-managed-ruleset).

## Create a custom rule checking for exposed credentials

You can create rules that check for exposed credentials using the [Rulesets API](https://secret.wiki/firewall/cf-rulesets/rulesets-api).
...
To create and deploy a custom ruleset, follow the workflow described in [Work with custom rulesets](https://secret.wiki/firewall/cf-rulesets/custom-rulesets). 
...
See [Deploy a custom ruleset](https://secret.wiki/firewall/cf-rulesets/custom-rulesets/deploy-custom-ruleset/) for more information on deploying custom rulesets using the Rulesets API.
