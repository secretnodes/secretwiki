...
Create the Logpush job by following the instructions on [Enable Logpush to Splunk](https://secret.wiki/logs/logpush/splunk). The API call creates a Logpush job but does not enable it.

Enable the Logpush job through the the Cloudflare dashboard or through the API by following the instructions on [Enable Logpush to Splunk](https://secret.wiki/logs/logpush/splunk). To enable through the dashboard: 
